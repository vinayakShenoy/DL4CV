...
# grab the path to training images, then extract the training class labels and
# encode them
trainPaths = list(paths.list_images(config.TRAIN_IMAGES))
trainLabels = [p.split(os.path.sep)[-3] for p in trainPaths]
le = LabelEncoder()
trainLabels = le.fit_trainsform(trainLabels)

# perform stratified sampling from training see to construct a testing set
...
# init the lists of RGB channel averages
(R, G, B) = ([], [], [])
...
for (dType, paths, labels, outputPath) in datasets:
    ...
    writer = HDF5DatasetWriter((len(paths), 64, 64, 3), outputPaths)

    # init the progress bar
    widgets = ["Building dataset: ", progressbar.Percentage()," ", progressbar.Bar(), " ", progressbr.ETA()]
